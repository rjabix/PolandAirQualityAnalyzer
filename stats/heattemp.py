...
def bycity(date1=None, date2=None):
    ...
    df = df[
        (df["temperature_avg"] > temp_start) &
        (df["temperature_avg"] < temp_end) &
    ...
    g = sns.JointGrid(data=df, x="temperature_avg", y="pm10_avg", space=0)
    ...
    g.set_axis_labels("Temperature (°C)", "PM 10")
    plt.show()
